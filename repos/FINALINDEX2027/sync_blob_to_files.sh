...
SRC="https://${AZ_STORAGE_ACCOUNT}.blob.core.windows.net/${AZ_BLOB_CONTAINER}/?${AZCOPY_BLOB_SAS}"
# IMPORTANT: trailing slash before `?` makes AzCopy treat this as a directory prefix
# (prevents `%2F`-encoded paths that break directory creation in Azure Files).
DST="https://${AZ_STORAGE_ACCOUNT}.file.core.windows.net/${AZ_FILES_SHARE}/${AZ_BLOB_CONTAINER}/?${AZCOPY_FILES_SAS}"

echo "Sync (Blob -> AzureFiles)"
echo "  src: ${AZ_STORAGE_ACCOUNT}/${AZ_BLOB_CONTAINER} (blob)"
echo "  dst: ${AZ_STORAGE_ACCOUNT}/${AZ_FILES_SHARE}/${AZ_BLOB_CONTAINER} (files)"
